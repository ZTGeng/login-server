...
exports.create_room = function (username, topic, rate, callback) {
    ...
    if (room_list[username]) {
        room_list[username] = {"roomId": roomId, 'topic': topic, 'rate': rate, 'free': true};
...
exports.get_rooms_by_topic = function (topic, callback) {
    var result = [];
    result.push({'username': "name1", 'topic': "topic1", 'rate': "3", 'roomId': '1234'});
    result.push({'username': "name2", 'topic': "topic2", 'rate': "4", 'roomId': '1235'});
    result.push({'username': "name3", 'topic': "topic3", 'rate': "5", 'roomId': '1236'});
    var count = 3;
    if (topic === 'General') {
    ...
    callback({'response':'Get List', 'size':count, 'list':JSON.stringify(result)});
}